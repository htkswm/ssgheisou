/* 背景演出（WebGL、ライブラリ不使用）
   ...
   - "window" : Link ページ。夜空（十二星神の設定画の色味）。深い藍〜青緑の空が揺らぎ、白い星と少数の赤い光点。星座は描かない。端に行くほど少し暗い。
...
(function () {
  ...
  var fs = [
    ...
    ' } else if(u_scene>3.5){',
    /* ---- Link ページ: 夜空（十二星神の設定画の色味。星座は描かない）。端に行くほど少し暗くなるだけ ---- */
    ...
    '  vec3 sky=vec3(0.02,0.06,0.16);',
    '  sky=mix(sky,vec3(0.06,0.32,0.42),smoothstep(0.38,0.75,neb));',
    '  sky=mix(sky,vec3(0.30,0.72,0.80),smoothstep(0.62,0.92,neb2)*0.6);',
    '  sky=mix(sky,vec3(0.10,0.05,0.18),smoothstep(0.15,0.0,neb)*0.5);',
    ...
    /* 周辺減光: 角丸四角に近い落ち方（横長でも引き伸ばされて見えない） */
    '  vec2 nq=vec2(abs(q.x)/(0.5*ar), abs(q.y)/0.5);',
    '  float dd=pow(pow(nq.x,4.0)+pow(nq.y,4.0),0.25);',
    '  float vg=1.0-0.62*smoothstep(0.55,1.08,dd);',
    '  rgb=sky*vg; a=0.88;',
    ' } else if(u_scene>2.5){',
